...
```
sudo apt install gcc-arm-none-eabi make cmake dfu-util
```
...
Команду `usbipd bind` можно выполнить только один раз, а затем при каждом подключении устройства выполнять только `usbipd attach`. Это может показаться неудобным, но такова плата за использование самой костыльной операционной системы на свете.
